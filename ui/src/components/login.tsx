...
export function Login(){
    ...
    const postUser = async () => {
        ...
        axios.post('http://localhost:5000/users/login', data, config).then(( reponse: AxiosResponse<{user: userBody, token: string}, any> )=>{
            console.log(reponse)
            navigate('/getTasks')
        }).catch((err)=>{
            console.log(err);
          ...
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6" action="#">
                  ...
                  <div>
                      ...
                      <input  onChange={ e => { setPassword(e.target.value)}} name="password" id="password" placeholder="••••••••" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required={true} />
                  </div>
